# app/main.py (refined: no extra spaces, full features intact)
...
def dashboard():
    ...
    for idx, (tab_name, plot_keys) in enumerate(plots):
        ...
        tab_id = f"tab{idx}"
        tab_headers += f"<li class='{active}' data-tab='{tab_id}'>{tab_name}</li>"
        tab_html = ""
        ...
        tab_contents += f"<div class='tab-content {active}' id='{tab_id}'>{tab_html}</div>"
